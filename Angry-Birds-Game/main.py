...
def draw_level_cleared():
    ...
    if level.number_of_birds >= 0 and len(pigs) == 0:
        ...
        pygame.draw.rect(screen, (75, 0, 130), rect)
...
def draw_level_failed():
    ...
    if level.number_of_birds <= 0 and time.time() - t2 > 5 and len(pigs) > 0:
        ...
        screen.blit(pig_happy, (400, 150))
        screen.blit(replay_button, (550, 480))
...
while running:
    ...
    pygame.display.flip()
    clock.tick(50)
    pygame.display.set_caption("Angry Birds - 2D Pygame + Pymunk Implementation")